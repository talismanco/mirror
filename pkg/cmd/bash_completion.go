...
func configureBashCompletion() error {
	bashCompletionFilePath := filepath.Join(config.Configuration.ConfigDirPath, "completion.bash")
	...
	bashrcText := `
# Enables command-line completion for mirror
source %s
`

	bashrcText = fmt.Sprintf(bashrcText, filepath.Join("$HOME", config.ConfigDirPath, "completion.bash"))
