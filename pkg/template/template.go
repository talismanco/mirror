...
func Get(path string) (Interface, error) {
	...
	ctxt, err := func(fname string) (map[string]interface{}, error) {
		...
		return metadata, nil
	}(filepath.Join(absPath, config.ContextFileName))

	metadataExists, err := osutil.FileExists(filepath.Join(absPath, config.TemplateMetadataName))
	if err != nil {
		return nil, err
	}
	...
	md, err := func() (Metadata, error) {
		...
		b, err := ioutil.ReadFile(filepath.Join(absPath, config.TemplateMetadataName))
		if err != nil {
			return Metadata{}, err
		}
	...
	return &dirTemplate{
		Context:  ctxt,
		FuncMap:  FuncMap,
		Path:     filepath.Join(absPath, config.TemplateDirName),
		Metadata: md,
	}, err
}
